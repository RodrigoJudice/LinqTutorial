namespace LinqTutorial.Examples
{
  public static class Contains
  {
    public static void Run()
    {
      ...
      var containsAnthony = Pets.Contains(new Pet(2, "Anthony", PetType.Cat, 2f));
      Console.WriteLine($"containsAnthony: {containsAnthony}");

      //let's store the reference to Hannibal pet
      var hannibal = Pets.First(); //selects the first element in the collection
      var containsHannibal = Pets.Contains(hannibal);
      Console.WriteLine($"containsHannibal: {containsHannibal}");

      //we can use any EqualityComparer we like
      var containsAnthonyWithEqualityComparer = Pets.Contains(
